...
//after -> show tradestories new entries, recommended, etc. 
...
function HomePage({setLocalStorage}) {
	...
	async function handleUpload() {
		...
		reader.addEventListener('loadend', async e => {
			...
			let trimmedData = await trimData(rawData);
		...
		<section className="about-section">
			...
			<div className="container">
		        ...
		        <div className="import-div">
		        	...
		        	<input className="input-field" onChange={ e => {setFile(e.target.files[0])} } type="file"/>
		        </div>
		        ...
		        <div className='status-div'>
		        	...
		        	<div className='done-text'>
			        	{done && <span> Go to <Link reloadDocument to='/bookshelves/all'>
				        								<u>BookShelves</u>
				        							</Link></span>}
			        </div>
		        </div>
